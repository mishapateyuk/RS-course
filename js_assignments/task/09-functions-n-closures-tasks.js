 ...
 */
function getPolynom() {
    let arg = Array.from(arguments).reverse();
    return function(x) {
        let result = null;
        for (var i = 0; i < arg.length; i++) {
            result += (arg[i] * Math.pow(x, i));
        }
        return result;
    }
}
...
function logger(func, logFunc) {
    return function() {
        let argToStr = JSON.stringify(Array.from(arguments)).slice(1, -1);
        logFunc(`${func.name}(${argToStr}) starts`);
        let result = func.apply(null, arguments)
        logFunc(`${func.name}(${argToStr}) ends`);
        return result;
    }
}
